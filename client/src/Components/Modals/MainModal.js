        ...
        <Dialog as='div' className='fixed inset-0 z-30 overflow-y-auto text-center' initialFocus={ cancelButtonRef } onClose={ () => setModalOpen(false) }>
          <div className='min-h-screen px-4'>
            ...
            <div className='absolute right-5 top-5'>
              <button onClick={ () => setModalOpen(false) } type='button' className='transitions justify-center w-10 h-10 flex-colo px-4 py-2 text-base font-medium text-subMain bg-white rounded-full hover:bg-subMain hover:text-mainText '>
                <IoClose />
              </button>
            </div>
          </div>
